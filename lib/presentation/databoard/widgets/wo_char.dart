...
class WOChart extends StatefulWidget {
  @override
  State<WOChart> createState() => _WOChartState();
}

class _WOChartState extends State<WOChart> {
  var controller = Get.find<WoReportController>();
  ...
  void initState() {
    ...
    percentages = controller.reportList.map<double>((report) {
      int woquantity = report.woquantity ?? 1;
      int totalQuantity = report.totalQuantity ?? 0;
      return woquantity > 0 ? (totalQuantity / woquantity) * 100 : 0.0;
    }).toList();
  }
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              "Tỉ lệ hoàn thành theo WO",
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
            ),
            SizedBox(height: 4),
            Text(
              "Tổng số lệnh sản xuất: ${controller.reportList.length}",
